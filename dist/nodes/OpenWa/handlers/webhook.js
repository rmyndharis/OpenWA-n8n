...
async function buildWebhookRequest(operation, itemIndex) {
    ...
    if (operation === 'update') {
        ...
        (0, params_1.assertFieldsResolved)(this, Object.fromEntries(Object.entries(updateFields).filter(([key, value]) => !(value === null && (key === 'filters' || key === 'headers')))), {
            active: 'Active',
        ...
        for (const key of ['url', 'events', 'active', 'retryCount']) {
            ...
            if (key === 'url') {
                ...
                if (!url) {
                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'URL cannot be empty', { itemIndex });
                }
                body.url = url;
                continue;
            }
