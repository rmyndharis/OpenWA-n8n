  ...
  webhookMethods = {
    default: {
      ...
      async create(this: IHookFunctions): Promise<boolean> {
        ...
        // n8n saves this node's static data only after every webhook trigger in the
        // workflow has registered, so an activation that fails at a later one leaves
        // a registration whose id was never saved: delete() cannot find it, and each
        // retry used to add another, every one delivering each event again. n8n keeps
        // this node's URL unique to it, so a registration carrying it belongs to this
        // Trigger; one made by hand with that URL is removed too, as the Trigger's
        // description says.
